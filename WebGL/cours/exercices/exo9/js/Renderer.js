...
Renderer.prototype.lightColor		= [1.0,1.0,1.0];
Renderer.prototype.ambiantColor		= [0.025,0.025,0.025];
Renderer.prototype.lightPos			= [0,3,-5];


Renderer.prototype.init	= function(context,viewportWidth,viewportHeight)
{
	this._ctx			= context;
...
Renderer.prototype._updateBuffers = function()
{
	...
	for( i = 0; i < max; i++ )
	{
		...
		normals = normals.concat(currentMesh.normales);
...
{
	var ldir = this.lightPos;
	//var ldir = vec3.create();
	//vec3.normalize(this.lightPos, ldir);
    //vec3.scale(ldir, -1);
	...
	this._ctx.uniform3fv(program.lightColorUniform, this.lightColor);
	this._ctx.uniform3fv(program.ambiantColorUniform, this.ambiantColor);
