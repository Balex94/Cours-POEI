...
Renderer.prototype.projMatrix			= null;
...
Renderer.prototype._renderTask		= function(task, scene, ctx)
{
	...
	for( j = 0; j < max2; j++ )
	{
		...
		ctx.uniformMatrix4fv(program.pMatrixUniform		, false	, this.projMatrix);
		...
		ctx.uniform3fv(program.specularColorUniform		, scene.pointLight.getSpecularColor());
		ctx.uniform3fv(program.lightPositionUniform		, scene.pointLight.getPosition());
		ctx.uniform3fv(program.lightColorUniform		, scene.pointLight.getColor());
		ctx.uniform3fv(program.ambiantColorUniform		, scene.ambientLight.getColor());
		ctx.uniform1f(program.alphaUniform				, mesh.alpha);
		ctx.uniform1f(program.shininessUniform			, mesh.material.shininess);
...
Renderer.prototype.resetViewport	= function(width,height)
{
	this.viewportWidth = width;
	this.viewportHeight = height;
	mat4.identity(this.projMatrix);
	mat4.perspective(45, this.viewportWidth / this.viewportHeight, 0.1, 100.0, this.projMatrix);
};
...
Renderer.prototype.clear 			= function(ctx, r, g, b, a)
{
...
Renderer.prototype.flush			= function(scene,ctx)
{
	...
	this.clear(ctx, scene.r, scene.g, scene.b, scene.a);
	//********************************** render opaques **************************
